<template>
  <q-page class="p-4">
    <PageCard icon="loyalty" title="Benefícios">
      <TableList
        title="Benefícios"
        titleButtonAdd="Novo Benefício"
        :column="columns"
        :rows="rows"
        :acoes="acoes"
    ...
    <q-dialog v-model="isModalOpen" persistent transition-show="scale" transition-hide="scale">
      <q-card class="bg-white rounded-2xl flex flex-col flex-nowrap" style="width: 820px; max-width: 90vw; max-height: 90vh;">
        ...
        <q-card-section class="flex-1 overflow-auto bg-gray-50 p-4">
          <div class="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-gray-100">
            <FormBeneficio v-if="isModalOpen" :beneficioId="currentId" :loading="salvando" @submitData="onSubmit" />
          </div>
        </q-card-section>
      </q-card>
    </q-dialog>
...
import FormBeneficio from 'components/forms/FormBeneficio.vue';
import {
  listarBeneficios,
  salvarBeneficio,
  deletarBeneficio,
} from 'src/services/beneficios/beneficiosAdminService';
...
const columns = [
  ...
  { name: 'destaque', label: 'Destaque', field: row => row.destaque ? 'Sim' : 'Não', align: 'left' },
...
async function onSubmit(data) {
  ...
  salvando.value = true;
  try {
    await salvarBeneficio(data, currentId.value);
    $q.notify({ type: 'positive', message: 'Benefício salvo com sucesso!' });
    isModalOpen.value = false;
    fetchData();
  } catch (error) {
    $q.notify({ type: 'negative', message: 'Erro ao salvar benefício.' });
  } finally {
...
async function fetchData(busca = '', pagina = 1) {
  try {
    const res = await listarBeneficios(pagina, itemsPerPage.value, busca);
    rows.value = res.data;
    maxPages.value = res.maxPag;
  } catch (error) {
    $q.notify({ type: 'negative', message: 'Erro ao carregar benefícios.' });
  }
}
...
function confirmDelete(row) {
  confirmDialog({
    ...
    try {
      await deletarBeneficio(row.id);
      $q.notify({ type: 'positive', message: 'Benefício excluído com sucesso.' });
      fetchData();
    } catch (e) {
      $q.notify({ type: 'negative', message: 'Erro ao excluir benefício.' });
    }
  });
}
