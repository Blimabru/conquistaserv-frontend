<template>
  <q-page class="p-4">
    <PageCard icon="apps" title="Serviços">
      <TableList
        title="Serviços"
        titleButtonAdd="Novo Serviço"
        :column="columns"
        :rows="rows"
        :acoes="acoes"
      ...
      <q-card class="bg-white rounded-2xl flex flex-col flex-nowrap" style="width: 700px; max-width: 90vw; max-height: 90vh;">
        ...
        <q-card-section class="flex-1 overflow-auto bg-gray-50 p-4">
          <div class="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-gray-100">
            <FormServico v-if="isModalOpen" :servicoId="currentId" :loading="salvando" @submitData="onSubmit" />
          </div>
        </q-card-section>
      </q-card>
    </q-dialog>
...
import FormServico from 'components/forms/FormServico.vue';
import {
  listarServicos,
  salvarServico,
  deletarServico,
} from 'src/services/servicos/servicosAdminService';
...
const columns = [
  ...
  { name: 'categoria', label: 'Categoria', field: row => row.categoria || '—', align: 'left' },
  { name: 'acessoRapido', label: 'Acesso rápido', field: row => row.acessoRapido ? 'Sim' : 'Não', align: 'left' },
...
async function onSubmit(data) {
  ...
  salvando.value = true;
  try {
    await salvarServico(data, currentId.value);
...
async function fetchData(busca = '', pagina = 1) {
  try {
    const res = await listarServicos(pagina, itemsPerPage.value, busca);
    rows.value = res.data;
